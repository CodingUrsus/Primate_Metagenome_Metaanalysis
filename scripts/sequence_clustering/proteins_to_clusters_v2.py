...
# python /ssd/hammera/IGC/primates/clust_test/seqs/proteins_to_clusters.py --protein_fastas /ssd/hammera/IGC/primates/clust_test/seqs/testdir/ --db_directory /ssd/hammera/IGC/bowtie/bowtie_indices/ --out /ssd/hammera/IGC/primates/clust_test/diamond_mapping/ --number_processors 10
...
def build_and_map_all_v_all(numb_procs, db_out, write_out, dir_of_fastas):
	...
	for a_fasta in all_the_fastas:
		...
		for fasta_search in all_the_fastas:
			...
			diamond_search_command = str("~/bin/diamond blastp -p " + str(numb_procs) + " -q " + fasta_search + " -d " + new_diamond_db + " -o " + write_out + query_host_name + "_against_" + db_host_name + ".txt" + " -f 6 qseqid sseqid qlen slen length qstart qend sstart send pident gaps evalue bitscore --evalue 0.000000000001 -k 50")
			print(diamond_search_command)
			os.system(diamond_search_command)
		remove_db = str("rm -f " + new_diamond_db)
		os.system(remove_db)
		rsync_and_clean_dir(write_out)
	return None



def build_and_map_to_humans(numb_procs, db_out, write_out, dir_of_fastas):
        all_the_fastas = get_file_names(dir_of_fastas, "faa")
        for a_fasta in all_the_fastas:
                new_diamond_db = "/ssd/hammera/IGC/bowtie/bowtie_indices/Homo_sapiens_0.9.24.dmnd"
                db_host_name = "Homo_sapiens"
                query_host_name = get_host_name(a_fasta)
                diamond_search_command = str("~/bin/diamond blastp -p " + str(numb_procs) + " -q " + a_fasta + " -d " + new_diamond_db + " -o " + write_out + query_host_name + "_against_" + db_host_name + ".txt" + " -f 6 qseqid sseqid qlen slen length qstart qend sstart send pident gaps evalue bitscore -k 1")
                print(diamond_search_command)
                os.system(diamond_search_command)
                remove_db = str("rm -f " + new_diamond_db)
                os.system(remove_db)
...
def rsync_and_clean_dir(some_out_dir):
	rsync_command = str("rsync -vru " + some_out_dir + "* /dfs/Sharpton_Lab/hammera/IGC/mcl_data/mcl_KEGG_seqs/")
	os.system(rsync_command)
	print(rsync_command)
	clean_directory = str("rm -f " + some_out_dir + "*.txt")
