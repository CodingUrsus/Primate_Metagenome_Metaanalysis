...
def map_reads_with_bowtie(number_of_processors, host_IGC, processed_directory, out_dir, bt_host_name):
	import subprocess
        output = subprocess.check_output(str("ls -d " + processed_directory + "*/fasta_cleaned/*R1_*"), shell=True)
	...
	for reads_file in new_obj:
		...
		reads_file_split = reads_file.split("/fasta_cleaned/")
		simple_name = reads_file_split[1]
		bowtie_mapping_command = ("bowtie2 -f " + "-k 1 " + "--met-file " + str(out_dir + simple_name + ".txt") + " --threads " + str(number_of_processors) + " -x " + str(host_IGC + bt_host_name) + " -1 " + reads_file + " -2 " + reads_file_2 + " -S " + str(out_dir + simple_name + ".sam") + str(" &> " + out_dir + simple_name + ".log"))
...
def sam_to_bam(sb_number_of_processors, sb_bowtie_dir):
	...
	for sam_file in sam_list:
		sam_file_abbrev_name = return_abbreviated_sample_name(sam_file, "_R1_", 0)
