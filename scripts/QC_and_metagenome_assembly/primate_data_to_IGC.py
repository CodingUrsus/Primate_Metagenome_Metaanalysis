...
## python ~/scripts/data_to_IGC.py --indir /ssd/hammera/IGC/ley_data/Canis_aureus/ --outdir /ssd/hammera/IGC/ley_data/outdir/ --number_processors 20 --host_genome /ssd/hammera/IGC/ley_data/Canis_aureus_small_genome.fna.gz --taxa_name Canis_aureus
import argparse, sys
import sys
import os
...
def run_shotcleaner(shotcleaner_indir, shotcleaner_outdir, shotcleaner_n_procs, bowtie_host_db_name):
	...
	shotcleaner_command = str("perl ~/scripts/run_shotcleaner.pl --indir=" + shotcleaner_indir + " --outdir=" + processed_outdir + " --nprocs=" + str(shotcleaner_n_procs) + " --db=/ssd/hammera/IGC/bowtie/bowtie_indices/ --dbname=" + bowtie_host_db_name)
...
##3#make_ssd_directory(taxa_name)	
